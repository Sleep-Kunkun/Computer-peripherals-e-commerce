...
from myapp.models import BThingWish
...
#         return APIResponse(code=1, msg='获取购物车失败')
@api_view(['GET'])
def getWishThingList(request):
    try:
        username = request.GET.get('username', None)
        if username:
            ...
            # 获取用户的所有愿望清单项以及对应的 count
            wish_things = user.wish_things.all()
            wish_counts = BThingWish.objects.filter(user=user).values('thing_id', 'count')
            
            # 将 count 信息添加到商品数据中
            thing_data = []
            for thing in wish_things:
                count = next((item['count'] for item in wish_counts if item['thing_id'] == thing.id), 0)
                thing_data.append({
                    'id': thing.id,
                    'classification_title': thing.classification.title,
                    'title': thing.title,
                    'cover': thing.cover.url if thing.cover else None,
                    'price': thing.price,
                    'status': thing.status,
                    'repertory': thing.repertory,
                    'score': thing.score,
                    'create_time': thing.create_time,
                    'pv': thing.pv,
                    'recommend_count': thing.recommend_count,
                    'wish_count': thing.wish_count,
                    'collect_count': thing.collect_count,
                    'classification': thing.classification.id,
                    'tag': [tag.id for tag in thing.tag.all()],
                    'count': count  # 添加 count 字段
                })
            
            return APIResponse(code=0, msg='操作成功', data=thing_data)
        else:
            return APIResponse(code=1, msg='username不能为空')
