<template>
  <div class="mine-infos-view">
    <div class="info-box flex-view">
      <img :src="userImg || AvatarImg" class="avatar-img" />
      <div class="name-box">
        <h2 class="nick">{{ userName }}</h2>
        <div class="age">
          <span>用户名：{{ userStore.user_name }}</span>
...
function getUserImg() {
  detailApi({ id: userStore.$state.user_id }).then(res => {
    console.log(res)
    userImg.value = BASE_URL + res.data.avatar
