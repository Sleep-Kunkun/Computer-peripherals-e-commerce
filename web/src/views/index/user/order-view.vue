      ...
      <div class="list-content">
        <div class="order-item-view" v-for="(item, index) in orderData" :key="index">
          <div class="header flex-view">
            ...
            <div class="right">
              <a-popconfirm v-if="item.status === '1'" title="确定取消订单？" ok-text="是" cancel-text="否"
                @confirm="handleCancel(item)">
                <a-button type="primary" size="small" style="margin-right: 24px">取消</a-button>
              </a-popconfirm>
              <a-popconfirm v-if="item.status === '3'" title="是否确定收货？" ok-text="是" cancel-text="否"
                @confirm="handleShouhuo(item)">
                <a-button type="primary" size="small" style="margin-right: 24px">确认收货</a-button>
              </a-popconfirm>
              <span class="text">订单状态</span>
              <span class="state" :style="{ color: item.status >= 3 ? 'green' : 'orange' }">{{ item.status === '1' ?
                '待支付' :
                item.status === '2' ? '已支付' : item.status === '3' ? '已发货' : item.status === '4' ? '已收货' : '已取消'
              }}</span>
            </div>
          </div>
          <div class="content flex-view">
            ...
            <div class="right-info">
              ...
              <p class="name">{{ item.receiver_name }}{{ item.receiver_phone }} </p>
              <p class="text mg">{{ item.receiver_address }} </p>
          ...
          <div class="bottom flex-view">
            <div class="left">
              <span class="text">共{{ item.count }}件外设</span>
              <span class="open" @click="handleDetail(item.thing)">外设详情</span>
            </div>
...
//确认收货
const handleShouhuo = (item) => {
  console.log(item.status)
  cancelApi({ id: item.id, status: 4 })
    .then((res) => {
