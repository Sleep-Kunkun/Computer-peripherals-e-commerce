<template>
  <div>
    ...
    <div class="page-view">
      ...
      <a-table size="middle" rowKey="id" :loading="data.loading" :columns="columns" :data-source="data.tagList"
        ...
        <template #bodyCell="{ text, record, index, column }">
          ...
          <template v-if="column.key === 'status'">
            <a-tag :color="text === '1' ? '#2db7f5' : '#87d068'">
              {{ text === '1' ? '待支付' : text === '2' ? '已支付' : text === '3' ? '已发货' : text === '4' ? '已收货' : '已取消' }}
            </a-tag>
          </template>
          <template v-if="column.key === 'operation'">
            <span class="Caozuo">
              <a-popconfirm title="确定发货?" ok-text="是" cancel-text="否" @confirm="confirmFaHuo(record)">
                <a>发货</a>
              </a-popconfirm>
...
const confirmFaHuo = (record: any) => {
  ...
  //将订单状态改为3 已发货
  cancelApi({ id: record.id, status: 3 })
    .then((res) => {
      getDataList();
